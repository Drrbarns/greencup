...
export async function POST(request: Request) {
  try {
    ...
    const appUrl = (process.env.NEXT_PUBLIC_APP_URL || 'https://greencup4u.com').replace(/\/$/, '');
    const resetUrl = `${appUrl}/auth/reset-password?token=${encodeURIComponent(token)}`;

    const apiKey = process.env.RESEND_API_KEY;
    const from = process.env.EMAIL_FROM || 'Green Cup <hello@greencup4u.com>';
    ...
    if (apiKey) {
      ...
      await resend.emails.send({
        from,
        to: user.email,
        subject: 'Reset your Green Cup password',
