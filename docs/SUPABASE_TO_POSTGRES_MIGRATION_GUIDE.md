# Supabase → Plain Postgres (VPS) — Green Cup

Playbook for this store. No secrets in this file.

Public URL: `https://greencup4u.com` (`NEXT_PUBLIC_APP_URL`).
...
FQDN: `https://greencup4u.com`, `https://www.greencup4u.com`, `https://upscalevintage.169-58-8-203.sslip.io`

Env (from `/data/fleet/secrets/store_upscalevintage.env`):

- `DATABASE_URL` (PgBouncer)
- `AUTH_SECRET`
- `NEXT_PUBLIC_APP_URL=https://greencup4u.com`
- `UPLOAD_DIR=/var/www/upscalevintage/uploads`
- `MOOLRE_CALLBACK_URL=https://greencup4u.com/api/payment/moolre/callback`
...
Set Moolre live keys (`MOOLRE_API_USER`, `MOOLRE_API_PUBKEY`, `MOOLRE_ACCOUNT_NUMBER`, `MOOLRE_CALLBACK_SECRET`) in Coolify when ready. Dashboard callback URL: `https://greencup4u.com/api/payment/moolre/callback`.
...
| `https://greencup4u.com/api/health` | Pass — database up, config ok |
